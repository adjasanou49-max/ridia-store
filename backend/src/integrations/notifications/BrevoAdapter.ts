  ...
  <tr><td align="center">
    <table role="presentation" width="480" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;">
      ...
      <tr><td style="padding:24px 32px 32px;color:#a1a1aa;font-size:12px;border-top:1px solid #f4f4f5;margin-top:16px;">
        Ridia Store — mode et essentiels du quotidien, livrés partout dans le monde.<br/>
...
/**
 * Remplace SendGridAdapter (compte rejeté par leur processus de vérification -
 * fréquent et arbitraire pour des comptes hors Europe/Amérique du Nord).
 * Brevo (ex-Sendinblue), entreprise française, beaucoup plus accessible pour
 * une petite entreprise ouest-africaine. Mêmes méthodes publiques que
 * SendGridAdapter pour ne rien casser ailleurs dans le code.
 */
export class BrevoAdapter {
  private get isMock() {
    return env.BREVO.mode !== 'live';
  }

  async sendEmail(to: string, subject: string, html: string): Promise<{ success: boolean }> {
    if (this.isMock) {
      logger.info('[Brevo MOCK] Sending email', { to, subject });
      return { success: true };
    }
    ...
    try {
      await axios.post(
        BREVO_API_URL,
        {
          sender: { email: env.BREVO.fromEmail, name: 'Ridia Store' },
          to: [{ email: to }],
          subject,
          htmlContent: html,
        },
      ...
      logger.error('Brevo send error', { error: err.response?.data ?? err.message });
  ...
  async sendWelcomeEmail(to: string, firstName: string): Promise<void> {
    const body = `
      ...
      <p style="margin:0 0 8px;">Ton compte est prêt. Découvre dès maintenant nos produits, livrés où que tu sois.</p>
    `;
    await this.sendEmail(
      to,
...
export const emailAdapter = new BrevoAdapter();
